...
async def on_message(message):
    try:
        ...
        if message.content=="jjh help" or message.content.startswith("jjh 도움말"):
            embed = discord.Embed(title = "명령어", color=0xd1173b)
            embed.add_field(name = "jjh 청소 n", value = "n에 적은 수 만큼 채팅을 삭제 할 수 있어여![메시지 관리권한 이상만 가능해여!]", inline = False)
            embed.add_field(name = "jjh 아재개그", value = "아재개그를 내뱉어여! 아핳하하ㅏㅎ", inline = False)
            embed.add_field(name = jjh 애교, value = "애교를 부려여! 뿌잉!", inline = False)
        ...
        if mc == "jjh 바보":
            await message.channel.send("제이봇 바부 아닌데! (울먹)")
        if mc == "jjh 애교":
            cryran = random.randint(0,4)
            if cryran == 0:
                await message.channel.send("뀨?")
            elif cryran == 1:
                await message.channel.send("왈왈!")
            elif cryran == 2:
                await message.channel.send("아옹?")
            elif cryran == 3:
                await message.channel.send("앙앙!")
            elif cryran == 4:
                await message.channel.send("쿨쿨...")
    except Exception as exe:
        await message.channel.send(":no_entry: ``에러 발생: "+str(exe)+"``")
